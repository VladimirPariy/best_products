...
const RegistrationModal: FC<Props> = (props) => {
  ...
  return (
    <ModalWrapper setVisible={setIsShowRegistrationModal} isVisible={isShowRegistrationModal}>
      <div className={styles.registrationContainer} onClick={e => e.stopPropagation()}>
        <ModalTitle>
          SIGN UP
        </ModalTitle>

        <ModalInput value={firstName} changeHandler={setFirstName} labelText='First name'/>
        <ModalInput value={lastName} changeHandler={setLastName} labelText='Last name'/>
        <ModalInput value={email} changeHandler={setEmail} labelText='Email address'/>
        <ModalInput value={password} changeHandler={setPassword} labelText='Password' type='password'>
          Password must contain at least five characters. A strong password contains a combination of letters, numbers and symbols.
        </ModalInput>
        <ModalInput value={confirmPassword} changeHandler={setConfirmPassword} labelText='Confirm password' type='password'/>
