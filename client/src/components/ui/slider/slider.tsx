        ...
        <div className={styles.previewImgContainer}>
          {
            (onDelete && images.length > 0) &&
						<button onClick={() => deleteHandler(images[imgSerialNr - 1].image_id)}
          ...
          {
            images.length  ?
              <img src={`${apiUrls.BASE_Image_URL}${images[imgSerialNr - 1]?.image_title}`}
                   alt="product"
            ...
            <div style={{marginLeft: `calc(${shiftingSlider}% )`}}
              ...
              {images.length > 0 && imgSerialNr > 0
                ? images.map((item, index) => (
                    <div className={index === imgSerialNr - 1 ? styles.active : undefined}
                         key={index}>
                      <img src={`${apiUrls.BASE_Image_URL}${item.image_title}`} alt="product"/>
                    </div>
                  )
                )
                : <div>
