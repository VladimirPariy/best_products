...
const UserAccModal: FC<Props> = ({isShowAccountModal, setIsShowAccountModal}) => {
  const {first_name, last_name, email, user_photo, is_get_update, phone_number, user_id: id} = useAppSelector(selectUser);
  ...
  const [previewPhoto, setPreviewPhoto] = useState<string | undefined | null>(user_photo);
  ...
  const updateHandling = () => {
    ...
    if (phone_number !== phone && phone.length > 0) updatingInfo = {...updatingInfo, phone_number: phone};
    if (is_get_update !== +getUpdates) updatingInfo = {...updatingInfo, is_get_update: +getUpdates};
    if (newPassword === confirmNewPassword && newPassword.length >= 5) updatingInfo = {...updatingInfo, password: newPassword};
    if (previewPhoto && previewPhoto !== user_photo) updatingInfo = {...updatingInfo, user_photo: uploadFile}
    ...
    <ModalWrapper setVisible={setIsShowAccountModal} isVisible={isShowAccountModal} isAccModal={true}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.userInfoContainer}>
          ...
          <ModalInput labelText='Phone' changeHandler={setPhone} value={phone} type='tel'/>
          ...
          <ModalButton submitHandler={updateHandling} type='button' children='Save All Changes'/>
        </div>
      </div>
