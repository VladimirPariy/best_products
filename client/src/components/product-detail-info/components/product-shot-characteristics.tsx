...
import styles from "components/product-detail-info/components/product-shot-characteristics.module.scss";
    ...
    <div className={styles.characteristicsWrapper}>
      {
        modifyParams.map(item => (
          <div key={item.parameter} className={styles.charContainer}>
            <span className={styles.charParam}>{upFirstChar(item.parameter)}: </span>
            <span className={styles.charNames}>{item.characteristics.join(', ')}</span>
          </div>
        ))
      }
    </div>
