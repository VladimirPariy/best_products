...
const ProductItem: FC<Props> = (props) => {
  ...
  return (
    <div className={searchParams.get('view') === 'list'
      ...
      <div className={styles.image}>
        <img src={`${apiUrls.BASE_Image_URL}${product_images[0].image_title}`}
             alt="product img"/>
      </div>
      {searchParams.get('view') === 'list' && (
          ...
          <div className={styles.characteristics}>
            {characteristics.map((char, index) => {
              ...
              <span className={styles.charTitle}>
                {char.parameters.parameter_title.at(0)?.toUpperCase()}{char.parameters.parameter_title.slice(1)?.toLowerCase()}:
              </span>
                  <span>
                {char.characteristic_title}
              </span>
                </div>
              )
            })}
