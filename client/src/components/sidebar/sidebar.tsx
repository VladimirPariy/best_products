...
const Sidebar: FC<Props> = ({setCheckedBurgerMenu}) => {
  ...
  const [sidebarSubcategories, setSidebarSubcategories] = useState<ISidebarList[]>([]);
  ...
  const categoryClickHandler = (e: MouseEvent<HTMLAnchorElement>, item: ISidebarList) => {
  ...
  const subcategoryClickHandler = (e: MouseEvent<HTMLAnchorElement>, item: ISidebarList) => {
    e.preventDefault()
    setIsOpenSubcategory(true)
    const category = categories.find(category => category.category_title === item.title)
    if (category) {
      const subcategory: ISidebarList[] = category.subcategories.map((subcategory, index) => {
  ...
  useEffect(() => {
    if (categories) {
      setSidebarCategories(categories.map((category) => {
        ...
        <div className={styles.extraCategory}>
          {
            isOpenCategory &&
              ...
              {sidebarCategories.map(category => (
                <SidebarItem onHover={onHover} item={category} key={category.id} clickHandler={subcategoryClickHandler}/>
              ))}
						</ul>
          }
          {(sidebarSubcategories && isOpenSubcategory) &&
              ...
              {sidebarSubcategories.map(subcategory => (
                ...
                <li key={subcategory.id} className={styles.subcategoryItem} onClick={() => setCheckedBurgerMenu && setCheckedBurgerMenu(false)}>
                  <NavLink to={subcategory.url} className={({isActive}) => isActive ? styles.active : null}>
                    <div>{subcategory.title}<span>{subcategory.id === 0 ? <Arrow/> : false}</span></div>
                  </NavLink>
                </li>
              ))}
						</ul>
