...
import {Chart, LineController, LineElement, PointElement, LinearScale, Title, CategoryScale, ChartData, ChartOptions} from 'chart.js';

import {clearPriceHistory, getPriceHistoryTrigger} from "store/price-history/price-history-actions";
import {selectPriceHistory, selectPriceHistoryError, selectPriceHistoryStatus} from "store/price-history/price-history-selectors";
...
const PriceHistoryTab: FC<Props> = ({product_id}) => {
  ...
  const [data, setData] = useState<ChartData<"line", string[], string>>({labels: [], datasets: []})
  ...
  return (
    <>
      ...
      {error && <ErrorContainer errorText='Some error has occurred. Please try again later'
                                style={{color: "rgba(252, 48, 3, 1)", textAlign: 'center'}}
      />}
      {priceHistory?.length > 0 && <div className={styles.chartContainer}>
        <Line data={data}/>
      </div>}
    </>
  );
