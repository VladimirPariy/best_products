...
class ProductsApi {
  ...
  async getProductDetail(id: number) {
    const {data} = await defaultAxios.get<IProduct>(`${apiUrls.one_product_by_id}${id}`);
    return data
  }

  async uploadFile({file, id}: IUploadImage) {
    const {data} = await authAxios.post<IProductImages>(`${apiUrls.one_product_by_id}${id}`, file);
    return data
  }
  ...
  async uploadTempImage(file: FormData) {
    const {data} = await authAxios.post<IProductImages>(apiUrls.temp_images, file);
    return data
  }

  async updateProductDetails({id, ...infoForUpdate}: UpdatingProductDetails) {
    return await authAxios.patch<string>(`${apiUrls.one_product_by_id}${id}`, infoForUpdate);
  }
