...
import {userRegistrationPending, userRegistrationFulfilled, userRegistrationTrigger, userRegistrationRejected} from "store/actions/user-registration-actions";
...
function* userRegistrationWorker(action: PayloadAction<IRegistrationData>) {
  ...
  try {
    const data: IReturningUserData = yield call(AuthApi.registration, action.payload)
    yield put(userRegistrationFulfilled(data))
  } catch (error) {
    if (error instanceof AxiosError) yield put(userRegistrationRejected({status_message: error.request.response, status: error.request.status}))
  }
}
