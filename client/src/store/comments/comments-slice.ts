...
export const commentsSlice = createSlice({
  ...
  reducers: {
    ...
    getCommentsFulfilled: (state, {payload}: PayloadAction<IShotCommentsWithUser[]>) => {
      state.status = false;
      state.error = null;
    ...
    addCommentFulfilled: (state, {payload}: PayloadAction<IShotCommentsWithUser>) => {
      state.error = null;
      state.status = false;
    ...
    removeCommentFulfilled: (state, {payload}: PayloadAction<{ id: number }>) => {
      const {id} = payload
      state.error = null;
      state.status = false;
      state.entities = state.entities.filter(item => item.comment_id !== id)
    },
    removeCommentRejected: (state, {payload}: PayloadAction<ErrorPayload>) => {
      state.error = payload;
      state.status = false;
    },
