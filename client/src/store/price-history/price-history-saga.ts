...
import {getPriceHistoryFulfilled, getPriceHistoryPending, getPriceHistoryRejected, getPriceHistoryTrigger} from "store/price-history/price-history-actions";

function* priceHistoryWorker({payload}: PayloadAction<number>) {
  yield put(getPriceHistoryPending());
  try {
    const res: IShotPriceHistory[] = yield call(
      priceHistoryApi,
      payload
    );
