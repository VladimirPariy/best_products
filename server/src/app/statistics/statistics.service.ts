...
class StatisticsService {
  ...
  async getMostFavorites() {
    return ProductsModel.query()
      .select([
        "products.product_id",
        "products.product_title",
        ProductService.getFavoriteAmount(),
      ])
  ...
  async getMostCommented() {
    return ProductsModel.query()
      .select([
        "products.product_id",
        "products.product_title",
        ProductService.getCommentsAmount(),
      ])
  ...
  async getWithHighestAverageRating() {
    const products = await ProductsModel.query().select([
      "products.product_id",
      "products.product_title",
      ProductService.getPositiveFeedbacksAmount(),
      ProductService.getNegativeFeedbacksAmount(),
    ]);
