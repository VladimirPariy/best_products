...
class ParametersController {
	async getParametersBySubcategoryId(req: Request, res: Response, next: NextFunction) {
		const {subcategoryId} = req.params
		if (!subcategoryId || isNaN(+subcategoryId)) {
			return next(HttpException.badRequest('Missing subcategory id'))
		}
		const data = await ParametersService.getParametersBySubcategoryId(+subcategoryId);
