...
class ProductService {
	...
	getProductOrProducts(id: number[], comment?: boolean): Objection.QueryBuilder<ProductsModel, ProductsModel[]>
	getProductOrProducts(id: number, comment?: boolean): Objection.QueryBuilder<ProductsModel, ProductsModel | undefined>
	getProductOrProducts(id?: number | number[], comment?: boolean) {
		const expr = "[product_images(selectShotImage), characteristics(selectShotCharacteristic).[parameters(selectShotParameter)], subcategories(selectShotSubcategory).[categories(selectShotCategory)]]"
		const colArray = [
			"products.*",
	...
	getPositiveFeedbacksAmount() {
		return ProductsModel.relatedQuery("feedbacks").count().as("positive_feedbacks_amount").where({feedback_type: 1})
	}
	getNegativeFeedbacksAmount() {
		return ProductsModel.relatedQuery("feedbacks").count().as("negative_feedbacks_amount").where({feedback_type: 2})
	}
	getCommentsAmount() {
